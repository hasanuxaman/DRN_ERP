...
{
    /// <summary>
    /// Summary description for getStaffPic
    /// </summary>
    public class getStaffPic : IHttpHandler
    {
        ...
        private byte[] GetImageFromDB(string EmpRefNo)
        {
            ...
            try
            {
                ...
                selcmd = new System.Data.SqlClient.SqlCommand("select Staff_Pic from tbl_TrTr_Staff_Img where Staff_Ref_No=" + EmpRefNo, conn);
                conn.Open();
                rdr = selcmd.ExecuteReader();
                while (rdr.Read())
                {
                    btImage = ((byte[])rdr["Staff_Pic"]);
                }
                if (rdr != null)
                {
                    rdr.Close();
