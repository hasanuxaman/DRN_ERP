...
{
    /// <summary>
    /// Summary description for getEmpSig
    /// </summary>
    public class getStaffDL : IHttpHandler
    {
        ...
        private byte[] GetImageFromDB(string EmpRefNo)
        {
            ...
            try
            {
                ...
                selcmd = new System.Data.SqlClient.SqlCommand("select Staff_DL from tbl_TrTr_Staff_Img where Staff_Ref_No=" + EmpRefNo, conn);
                conn.Open();
                rdr = selcmd.ExecuteReader();
                while (rdr.Read())
                {
                    btImage = ((byte[])rdr["Staff_DL"]);
                }
                if (rdr != null)
                {
                    rdr.Close();
