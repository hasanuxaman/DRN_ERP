...
{
    public partial class frmQuotCsInq : System.Web.UI.Page
    {
        ...
        private void generate_comments(string ref_no)
        {
            ...
            if (qtnSeqNo == 1) 
                lblQtnStat.Text = "C/S Status: Waiting for Audit Approval.";
            else if (qtnSeqNo == 2)
                lblQtnStat.Text = "C/S Status: Waiting for Purchase Order Approval.";
            else if (qtnSeqNo == 3)
                lblQtnStat.Text = "C/S Status: Waiting for Purchase Order.";
            else if (qtnSeqNo == 4)
                lblQtnStat.Text = "C/S Status: Purchase Order Created.";
            else
                lblQtnStat.Text = "";
        }
        ...
        private void get_pending()
        {
            ...
            if (cboMonth.SelectedIndex == 0)
                dtdet = indet.GetDataByQtnListByYear(Convert.ToDecimal(cboYear.SelectedValue.ToString()));
            else
                dtdet = indet.GetDataByQtnListByYearMonth(Convert.ToDecimal(cboYear.SelectedValue.ToString()), Convert.ToDecimal(cboMonth.SelectedValue.ToString()));            

            ddllist.Items.Clear();
            foreach (dsProcTran.tbl_PuTr_Pr_DetRow dr in dtdet.Rows)
            {
                lst = new ListItem();
                lst.Text = dr.Pr_Det_Quot_Ref.ToString() + ":" + dr.Pr_Det_Ref.ToString() + ":" + dr.Pr_Det_Itm_Desc.ToString() + "    [" + dr.Pr_Det_Lin_Qty.ToString() + " " + dr.Pr_Det_Itm_Uom.ToString() + "]";
