...
namespace DRN_WEB_ERP.Module.HRMS.Tools
{
    ...
    /// </summary>
    public class getEmpSig : IHttpHandler
    {
        ...
        private byte[] GetImageFromDB(string EmpRefNo)
        {
            ...
            try
            {
                ...
                selcmd = new System.Data.SqlClient.SqlCommand("select EmpImgSig from tblHrmsEmpImg where EmpImgEmpRefNo=" + EmpRefNo, conn);
                conn.Open();
                rdr = selcmd.ExecuteReader();
                while (rdr.Read())
                {
                    btImage = ((byte[])rdr["EmpImgSig"]);
                }
                if (rdr != null)
                {
                    rdr.Close();
