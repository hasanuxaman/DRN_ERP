...
{
    /// <summary>
    /// Summary description for getEmpPic
    /// </summary>
    public class getEmpPic : IHttpHandler
    {
        ...
        private byte[] GetImageFromDB(string EmpRefNo)
        {
            ...
            try
            {
                ...
                selcmd = new System.Data.SqlClient.SqlCommand("select EmpImgPic from tblHrmsEmpImg where EmpImgEmpRefNo=" + EmpRefNo, conn);
                conn.Open();
                rdr = selcmd.ExecuteReader();
                while (rdr.Read())
                {
                    btImage = ((byte[])rdr["EmpImgPic"]);
                }
                if (rdr != null)
                {
                    rdr.Close();
