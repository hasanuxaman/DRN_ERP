...
namespace DRN_WIN_ERP
{
    public partial class frmBrowser : Form
    {
        ...
        public frmBrowser(string ConnectionStr, string QueryStr, string GridColumnName)
        {
            InitializeComponent();
        ...
        private void SearchByText(string str)
        {
            ...
            try
            {
                string selectQuery = PreparQueryStr(QueryString, str);
        ...
        private string PreparQueryStr(string qrystr, string searchstr)
        {
            string qstring = RemoveSpace(qrystr.ToLower());
            string str1 = "", str2 = "", str3 = "", str4 = "", retStr = "";
            if (qstring.IndexOf("order by") > 0)
            {
                str1 = qstring.Substring(0, qstring.IndexOf("order by"));
                str2 = qstring.Substring(qstring.IndexOf("order by") + ("order by").Length);

                if (qstring.IndexOf("where") > 0)
                {
                    str3 = str1.Substring(0, str1.IndexOf("where"));
                    str4 = str1.Substring(str1.IndexOf("where") + ("where").Length);

                    retStr = str3 + " where " + str4 + " and " + TableColumnName[searchColumn] + " like '" + searchstr + "%' order by " + TableColumnName[searchColumn];

                }
                else
                {
                    retStr = str1 + " where " + TableColumnName[searchColumn] + " like '%" + searchstr + "' order by " + TableColumnName[searchColumn];
                }
            }
            else
            {
                ...
                {
                    retStr = qstring + " where " + TableColumnName[searchColumn] + " like '" + searchstr + "%' order by " + TableColumnName[searchColumn];
                }
            }
